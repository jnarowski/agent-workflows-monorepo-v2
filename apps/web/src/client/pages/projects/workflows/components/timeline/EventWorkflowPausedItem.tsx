...
import type { EventWorkflowPausedItemProps } from "./types";

/**
 * Workflow Paused event timeline item
 * Shows pause reason if provided
 */
export function EventWorkflowPausedItem({ event }: EventWorkflowPausedItemProps) {
  const [isExpanded, setIsExpanded] = useState(false);
  const { title, reason } = event.event_data;
  const config = getEventConfig("workflow_paused");
