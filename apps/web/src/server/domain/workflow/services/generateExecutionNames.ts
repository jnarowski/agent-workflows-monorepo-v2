...
/**
 * Options for generating execution names
 */
export interface GenerateExecutionNamesOptions {
 ...
 */
export interface GenerateExecutionNamesResult {
  /** 3-6 word descriptive execution name in Title Case */
...
/**
 * Generate execution name and branch name from spec file content
 *
 * Uses AI to extract the main goal/feature from the spec and create:
 * - Execution name: 3-6 words, Title Case, descriptive
 ...
 *
 * @example
 * const result = await generateExecutionNames({
 *   specContent: "# Spec\n\nFix authentication flow bug..."
 * });
 * // Returns: {
 * //   executionName: "Fix Auth Flow Bug",
 * //   branchName: "fix-auth-flow"
 * // }
 */
export async function generateExecutionNames(
  options: GenerateExecutionNamesOptions
): Promise<GenerateExecutionNamesResult | null> {
  ...
  try {
    ...
    const schema = z.object({
      executionName: z
        .string()
        .describe("3-6 word descriptive execution name in Title Case"),
      branchName: z
        .string()
    ...
    const result = await generateObject({
      ...
      system: `You analyze specification documents and create concise names for workflow executions and git branches.

Rules for execution names:
