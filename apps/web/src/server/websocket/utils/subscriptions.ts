...
export function subscribe(channelId: string, socket: WebSocket): void {
  ...
  console.log(
    `[Subscriptions] Socket subscribed to ${channelId} (${subscribers.size} total subscribers)`
  );
}
...
export function unsubscribe(channelId: string, socket: WebSocket): void {
  ...
  if (subscribers) {
    ...
    console.log(
      `[Subscriptions] Socket unsubscribed from ${channelId} (${subscribers.size} remaining)`
    );
  }
...
export function unsubscribeAll(socket: WebSocket): void {
  ...
  console.log(
    `[Subscriptions] Socket unsubscribed from ${channelCount} channels`
  );
}
...
export function broadcast(
  ...
  if (!subscribers || subscribers.size === 0) {
    console.log(
      `[Subscriptions] Broadcast to ${channelId} - no subscribers`
    );
    return;
  }
  ...
  for (const socket of subscribers) {
    if (socket.readyState === 1) {
      ...
      try {
        ...
        console.error(
          `[Subscriptions] Failed to send to socket in ${channelId}:`,
          error
        );
  ...
  if (deadSockets.length > 0) {
    ...
    console.log(
      `[Subscriptions] Removed ${deadSockets.length} dead sockets from ${channelId}`
    );
  }

  console.log(
    `[Subscriptions] Broadcast to ${channelId} - ${subscribers.size - deadSockets.length} delivered`
  );
}
