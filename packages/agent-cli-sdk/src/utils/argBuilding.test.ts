...
describe('argBuilding', () => {
	describe('permissionModeToFlags', () => {
		...
		it('should return ["--bypass-permissions"] for "bypassPermissions" mode', () => {
			expect(permissionModeToFlags('bypassPermissions')).toEqual([
				'--bypass-permissions',
			]);
		});
		...
		it('should return array that can be spread into args', () => {
			...
			expect([...baseArgs, ...flagsForAcceptEdits]).toEqual([
				'prompt text',
				'--accept-edits',
			]);
		});
	...
	describe('workingDirToFlags', () => {
		...
		it('should return ["--cwd", path] for valid path', () => {
			expect(workingDirToFlags('/path/to/dir')).toEqual([
				'--cwd',
				'/path/to/dir',
			]);
		});
		...
		it('should handle absolute paths', () => {
			expect(workingDirToFlags('/usr/local/project')).toEqual([
				'--cwd',
				'/usr/local/project',
			]);
		});
		...
		it('should handle paths with spaces', () => {
			expect(workingDirToFlags('/path with spaces/project')).toEqual([
				'--cwd',
		...
		it('should handle Windows paths', () => {
			expect(workingDirToFlags('C:\\Users\\Project')).toEqual([
				'--cwd',
				'C:\\Users\\Project',
			]);
		});
	...
	describe('combined usage', () => {
		it('should combine both utilities for full arg building', () => {
			...
			const args = [
				prompt,
				...workingDirToFlags(cwd),
				...permissionModeToFlags(mode),
			];
		...
		it('should work with no optional flags', () => {
			...
			const args = [
				prompt,
				...workingDirToFlags(undefined),
				...permissionModeToFlags(undefined),
			];
